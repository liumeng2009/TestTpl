    ...
    $scope.doLogin=function(){
      ...
      $loginData.login(this.user).success(function(data){
        ...
        if(data.success === 0){
          ...
          document.addEventListener('deviceready', function() {
            ...
            db.executeSql('select count(*) AS mycount from users where id=?',[data.user._id],function(rs){
              ...
              if(exist){
                ...
                db.transaction(function(tx){
                  tx.executeSql('update users set active=0');
                  tx.executeSql('insert into users values(?,?,?,?,?,?)',[data.user._id,data.user.name,1,data.user.image,data.user.token,data.user.meta.createAt]);
                });
              }
            });
          });
          ...
          //登陆成功，调用推送服务
          window.pushservice.startService({name:data.user.name,_id:data.user._id,image:data.user.image,token:data.user.token,createAt:data.user.meta.createAt});
          //登录成功之后，跳转
